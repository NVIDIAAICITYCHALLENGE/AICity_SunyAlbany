...
def draw_bounding_box_on_image(image,
  ...
  #print 'box:', xmin, xmax, ymin, ymax, im_height, im_width  
  #print 'box:', int(left), int(top), int(right-left), int(bottom-top)   
...
def visualize_boxes_and_labels_on_image_array(image,
  ...
  boxes_out = []
  scores_out = []
  class_out = []    # YI, store class names
  im_height = image[:,:,0].shape[0]
  im_width = image[:,:,0].shape[1]
  #im_width, im_height = image[:,:,0].size
  if not max_boxes_to_draw:
    max_boxes_to_draw = boxes.shape[0]
  for i in range(min(max_boxes_to_draw, boxes.shape[0])):
    if scores is None or scores[i] > min_score_thresh:
      box = tuple(boxes[i].tolist())
      #print 'box---:', box, '-----scores:', scores[i]
      (left, right, top, bottom) = (box[1] * im_width, box[3] * im_width, box[0] * im_height, box[2] * im_height)
      #YI:
      bt = (left, top, right, bottom)
      #bt = (left, top, right-left, bottom-top)
      boxes_out.append(bt)
      scores_out.append(scores[i])
      if classes[i] in category_index.keys():
        classn = category_index[classes[i]]['name']
        class_out.append(classn)
      else:
  ...
  #print "box %d score %d class %d" %(len(boxes_out), len(scores_out), len(class_out))
  return boxes_out, scores_out, class_out

...
boolean graph_write_dimacs_ascii_file(graph_t *g, char *comment, char *file) {
	FILE *fp;
	// YI: errno_t not defined, change to int
	int err;

	ASSERT((sizeof(setelement)*8)==ELEMENTSIZE);
	ASSERT(file!=NULL);
	// YI: change fopen_s to fopen
	/*
...
boolean graph_write_dimacs_binary(graph_t *g, char *comment,FILE *fp) {
	...
	headersize=1024;
	if (comment) {
		// YI change _s function to linux std function
		/*
		strcpy_s(buf, MAX(1024,g->n/8+1), "c ");
		strncat_s(buf, MAX(1024,g->n/8+1), comment,1000);
		strcat_s(buf, MAX(1024,g->n/8+1), "\n");
		STR_APPEND(buf);
		*/
		strcpy(buf,  "c ");
	...
	//sprintf_s(buf, MAX(1024,g->n/8+1), "p edge %d %d\n",g->n,graph_edge_count(g));
	sprintf(buf, "p edge %d %d\n",g->n,graph_edge_count(g));
	STR_APPEND(buf);
	for (i=0; i < g->n; i++) {
		if (g->weights[i]!=1) {
			//sprintf_s(buf, MAX(1024,g->n/8+1), "n %d %d\n",i+1,g->weights[i]);
			sprintf(buf, "n %d %d\n",i+1,g->weights[i]);
			STR_APPEND(buf);
		}
	}
...
graph_t *graph_read_dimacs_file(char *file) {
	...
	/*
	if ((err=fopen_s(&fp, file,"rb"))==NULL) {
		perror(file);
		return NULL;
	}
	*/
...
graph_t *graph_read_dimacs(FILE *fp) {
	...
	// YI: sscanf_s _> sscanf
	//if (sscanf_s(buffer," %d %2s",&n,tmp)!=1) {

...
#define SET_CONTAINS_FAST(s,a) (SET_ELEMENT_CONTAINS((s)[(a)/ELEMENTSIZE], \
						      (a)%ELEMENTSIZE))
#define SET_CONTAINS(s,a) (((double)(a)<(double)SET_MAX_SIZE(s))?SET_CONTAINS_FAST(s,a):FALSE)
...
static void set_print(set_t s) {
	...
	for (i=0; i<(int)SET_MAX_SIZE(s); i++)
		if (SET_CONTAINS(s,i))
			printf(" %d",i);
	printf("\n");
	return;
